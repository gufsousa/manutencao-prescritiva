...
def _candidate_reference(reference_slice: pd.DataFrame, event: dict[str, Any], top_k: int = TOP_K) -> pd.DataFrame:
...
def predict_euclidean_knn(event: dict[str, Any], reference_df: pd.DataFrame, scaler: StandardScaler) -> dict[str, Any]:
    ...
    candidate_df = _candidate_reference(reference_df, event)
...
def predict_mahalanobis_weighted_knn(event: dict[str, Any], reference_df: pd.DataFrame, scaler: StandardScaler) -> dict[str, Any]:
    ...
    ranked = (
        candidate_df.assign(score=distances, vote_weight=weights)
        .sort_values("score")
        .head(TOP_K)
    )
...
def predict_centroid_euclidean(event: dict[str, Any], reference_df: pd.DataFrame, scaler: StandardScaler, centroid_model: NearestCentroid) -> dict[str, Any]:
    started = perf_counter()
    event_frame = pd.DataFrame([{feature: event.get(feature, np.nan) for feature in FEATURE_COLUMNS}]).fillna(
        reference_df[FEATURE_COLUMNS].median()
    )
...
Classes permitidas: {", ".join(BENCHMARK_FAMILIES)}
...
def run_benchmark() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    centroid_model: NearestCentroid = artifacts["centroid_model"]
    ...
    for sample_index, (_, row) in enumerate(benchmark_df.iterrows(), start=1):
        ...
        for technique, provider, model in llm_variants:
            ...
            result = predict_llm_vector_rag(event, provider, model, reference_text_records)
            records.append(
                {
                    "sample_id": int(row["id"]),
                    "true_fault": true_fault,
